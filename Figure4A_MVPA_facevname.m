%TDT based code to classify faces vs name during preparation, separately in
...
for sub = sub_real
    for event = {'cuejit'}
        for b = 1:2
            ...
            resultsdir = [home subdir '/MVPA/Cue/' block{b} '/Faces_vs_Name/'];
            ...
            labelname1 = [event{1} '*' block{1,b} '*' levels{1} '*'];
            labelname2 = [event{1} '*' block{1,b} '*' levels{2} '*'];
...
%% Second Level
%% A. Preprocess the resulting images (normalize, smooth and mask)
...
for s = sub_real
    cell_scans{s,1} = ['E:\AttExp_fMRI\Data\derivatives\sub-' num2str(sub_list(s),'%.3d') '/MVPA/Cue/Att/Faces_vs_Name/masked_swres_balanced_accuracy_minus_chance.nii,1'];
end
...
cell_scans = cell(length(sub_real),1);
for s = sub_real
    cell_scans{s,1} = ['E:\AttExp_fMRI\Data\derivatives\sub-' num2str(sub_list(s),'%.3d') '/MVPA/Cue/Exp/Faces_vs_Name\masked_swres_balanced_accuracy_minus_chance.nii,1'];
end
cell_scans = cell_scans(~cellfun('isempty',cell_scans));
