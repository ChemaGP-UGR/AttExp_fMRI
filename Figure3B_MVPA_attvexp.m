%TDT based code to perfom decoding between attention and expectation cues
...
for sub = sub_real 
    for event =  {'target'}
        for model = 1
            ...
            cfg.plot_design = 0;
            cfg.analysis = 'searchlight' ; %or ROI
            ...
            if strcmp(cfg.analysis, 'ROI')
                resultsdir = [home subdir '/MVPA/roi/' roi{r} '/Cue/' variable{1} '/'];
            else
                resultsdir = [home subdir '/MVPA/Cue/' variable{1} '/'];
            end
            ...
            beta_loc = [home subdir '/GLM_models/decoding/']; %where the betas are
            
            % Mask to restrict the analysis to the participant's brain
            % (native space, unsmoothed)
            %
            if  strcmp(cfg.analysis, 'ROI')
                cfg.files.mask = [home subdir '\anat\rois\bin_rw' roi{r} '.nii'];
            else
            ...
            labelname1 = [event{1} '*' levels{model,1} '*'];
            labelname2 = [event{1} '*' levels{model,2} '*'];
            cfg = decoding_describe_data(cfg,{labelname1 labelname2},[1 -1],regressor_names,beta_loc);
            
                 %here we rename  the chunks so that in every cv loop
%                     there are a block of each left. This is because here
%                     run = block. 

            if size(cfg.files.chunk,1) == 14
                if cfg.files.chunk(1) == 2 || sub==46  %the missing block makes it fit this configuration
                          cfg.files.chunk = cfg.files.chunk([1:12]); 
                          cfg.files.label = cfg.files.label([1:12]);
                          cfg.files.chunk(1:6) = cfg.files.chunk(7:12);   
                    elseif cfg.files.chunk(1) == 1;
                         cfg.files.chunk = cfg.files.chunk([1:4 7:14]); 
                         cfg.files.label = cfg.files.label([1:4 7:14]);
                         cfg.files.chunk(7:12) = cfg.files.chunk(1:6);  
                    elseif cfg.files.chunk(1) == 3;
                         cfg.files.chunk = cfg.files.chunk([1:6 9:14]); 
                         cfg.files.label = cfg.files.label([1:6 9:14]);
                         cfg.files.chunk(7:12) = cfg.files.chunk(1:6);  
                    end
                else
                    if cfg.files.chunk(1)==1
                        cfg.files.chunk(9:16) = cfg.files.chunk(1:8);  
                    else
                        cfg.files.chunk(1:8) = cfg.files.chunk(9:16); 
                    end
            end
...
for s = sub_real
    cell_scans{s,1} = ['E:\AttExp_fMRI\Data\derivatives\sub-' num2str(sub_list(s),'%.3d') '\MVPA\Cue\Att_vs_Exp\masked_swres_balanced_accuracy_minus_chance.nii,1'];
end
