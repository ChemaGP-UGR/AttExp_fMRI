%TDT based code to cross-classify faces vs name during preparation,
%training in the cue and testing in the target, separateley for 
...
levels_t = {'Face','Word'};
cue_n = {'1', '2'};
% run_miss =  [1 8 14 16 24 40 42 45 46 47 48]; 
count = 1;
event = {'cuejit', 'target'};
for sub = sub_real
    for b = 1:2
            ...
            resultsdir = [home subdir '/MVCC/Cue_vs_Target_cv/' block{1,b} '/Train_Cue'];
            ...
            %prepare the labels for the analysis
            labelname2classA = ['*' event{1} '*' block{1,b} '*' levels{1} '*'];
            labelname2classB = ['*' event{1} '*' block{1,b} '*' levels{2} '*'];
            labelname1classA = ['*' event{2} '*' block{1,b} '*' levels_t{1} '*'];
            labelname1classB = ['*' event{2} '*' block{1,b} '*' levels_t{2} '*'];
            ...
            cfg.files.twoway = 0; %we train in the cue, and test in the target
            % changing labels to separate training and testing by runs
            
            
            % cfg.design
            cfg.design = make_design_xclass_cv(cfg); % for crossvalidation add _cv
...
for s = sub_real
    cell_scans{s,1} = ['E:\AttExp_fMRI\Data\derivatives\sub-' num2str(sub_list(s),'%.3d') '\MVCC\Cue_vs_Target\Att\Train_Cue\masked_swres_balanced_accuracy_minus_chance.nii,1'];
end
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Att';
...
cell_scans = cell(length(sub_real),1);
for s = sub_real
    cell_scans{s,1} = ['E:\AttExp_fMRI\Data\derivatives\sub-' num2str(sub_list(s),'%.3d') '\MVCC\Cue_vs_Target\Exp\Train_Cue\masked_swres_balanced_accuracy_minus_chance.nii,1'];
end
cell_scans = cell_scans(~cellfun('isempty',cell_scans));
matlabbatch{1}.spm.stats.factorial_design.dir = {'E:\AttExp_fMRI\Data\derivatives\second_level\multivariate\MVCC\Cue_vs_Target\Exp\Train_Cue'};
matlabbatch{1}.spm.stats.factorial_design.des.t1.scans = cell_scans;
matlabbatch{1}.spm.stats.factorial_design.cov = struct('c', {}, 'cname', {}, 'iCFI', {}, 'iCC', {});
matlabbatch{1}.spm.stats.factorial_design.multi_cov = struct('files', {}, 'iCFI', {}, 'iCC', {});
matlabbatch{1}.spm.stats.factorial_design.masking.tm.tm_none = 1;
...
matlabbatch{2}.spm.stats.fmri_est.method.Classical = 1;
matlabbatch{3}.spm.stats.con.spmmat(1) = cfg_dep('Model estimation: SPM.mat File', substruct('.','val', '{}',{2}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','spmmat'));
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'Exp';
matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = 1;
matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
matlabbatch{3}.spm.stats.con.delete = 0;
spm_jobman('run', matlabbatch);
